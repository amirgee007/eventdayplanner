            ...
            if ($date) {
                $events = Event::search($query)
                    /*->Join('bookings', function($join) use($date){
                                     ...
                                     })->select(array('ads.*','ads_id','book_date'))*/
                    ->Where('location', 'like', '%' . $location . '%')->groupBy('events.id')->orderBy('date')->paginate(3);
        ...
        if ($iseventsearch) {
            if ($date) {
                $events = Event::search($query)
                    ...
                    ->Where('location', 'like', '%' . $location . '%')->groupBy('events.id')->paginate(15);
